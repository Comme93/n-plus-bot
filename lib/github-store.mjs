...
function mergeRemindersState(incoming, fresh, inWin) {
  if (inWin) {
    return mergeRemindersForDaemon(incoming.reminders || [], fresh.reminders || []);
  }
  const union = unionReminders(fresh.reminders, incoming.reminders);
  return mergeRemindersForDaemon(union, incoming.reminders || []);
}
...
export function mergeState(incoming, fresh) {
  ...
  return {
    ...
    reminders: mergeRemindersState(incoming, fresh, inWin),
    ui,

...
export async function mergeForDaemonSave(token, workerData) {
  ...
  const reminders = unionReminders(fresh.data.reminders, workerData.reminders);
  ...
  return {
    data: {
      ...
      drafts: mergeDrafts(fresh.data.drafts, workerData.drafts),
      reminders: mergeRemindersForDaemon(reminders, workerData.reminders),

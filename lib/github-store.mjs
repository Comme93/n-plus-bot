...
export async function mergeForDaemonSave(token, workerData) {
  ...
  return {
    data: {
      ...
      ui: mergeUi(workerData.ui, fresh.data.ui),
      drafts: mergeDrafts(workerData.drafts, fresh.data.drafts),
...
function pickOnConflict(incoming, fresh) {
  ...
  return {
    ...
    seen: incoming.seen?.length ? incoming.seen : fresh.seen,
    seeded: incoming.seeded ?? fresh.seeded,
    reminders: inWin ? incoming.reminders : mergeRemindersForDaemon(fresh.reminders, incoming.reminders),
    ui,
    drafts: inWin ? mergeDrafts(fresh.drafts, incoming.drafts) : mergeDrafts(incoming.drafts, fresh.drafts),
...
export async function saveToGitHub(token, data, sha) {
  ...
  if (!res.ok) {
    if (res.status === 409 && sha) {
      const fresh = await loadFromGitHub(token);
      return saveToGitHub(token, pickOnConflict(data, fresh.data), fresh.sha);
    }

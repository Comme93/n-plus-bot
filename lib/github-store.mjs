  ...
  if (!res.ok) {
    if (res.status === 409 && sha) {
      ...
      data = {
        ...
        sessions: { ...(fresh.data.sessions || {}), ...(data.sessions || {}) },
        pending: { ...(fresh.data.pending || {}), ...(data.pending || {}) },
        panels: { ...(fresh.data.panels || {}), ...(data.panels || {}) },
        reminders: mergeReminders(fresh.data.reminders, data.reminders),
        seen: fresh.data.seen?.length ? fresh.data.seen : data.seen,
        seeded: fresh.data.seeded ?? data.seeded,
...
export async function mergeEphemeral(token, data) {
  const fresh = await loadFromGitHub(token);
  if (!fresh) return { data, sha: null };
  return {
    data: {
      ...data,

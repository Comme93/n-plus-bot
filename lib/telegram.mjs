...
function whenKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: 'Раз', callback_data: 'rem:once' }],
    ],
  };
}
...
async function handleCallback(data, cb, api) {
  ...
  if (d === 'rem:once') {
    ...
    if (s?.step === 'new_when' && s.text) {
      data.sessions[sid(chatId)] = { step: 'new_time', text: s.text };
...
async function handleMessage(data, msg, api) {
  ...
  if (s?.step === 'new_text') {
    data.sessions[sid(chatId)] = { step: 'new_when', text };
    await api('sendMessage', {
      chat_id: chatId,
      text: 'Когда',
  ...
  if (s?.step === 'new_time' && s.text) {
    ...
    if (!at) {
      await api('sendMessage', { chat_id: chatId, text: 'Формат: 20:30 или завтра 20:30' });
      return data;
    }
    data.reminders.push({
      id: rid(),

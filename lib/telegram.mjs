import { parseMskTime, parseClock, formatMsk, clockStr, mskParts, mskDayKey, onceTodayAt, isTodayMsk } from './time.mjs';
...
async function handleCallback(data, cb, api) {
  ...
  if (d === 'mode:today' && w?.text && w.hour != null) {
    const at = onceTodayAt(w.hour, w.minute);
    if (!at) {
      await api('sendMessage', { chat_id: chatId, text: 'Уже прошло' });
      return data;
    }
    addOnce(data, chatId, w.text, at);
    return done(data, chatId, api);
  }
